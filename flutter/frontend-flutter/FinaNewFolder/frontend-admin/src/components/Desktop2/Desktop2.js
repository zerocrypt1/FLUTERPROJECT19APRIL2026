...
function Desktop2() {
  ...
  // --- FIX 1: Use useRef to prevent double execution in React Strict Mode ---
  ...
  const fetchForms = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // This toast is the side effect causing the double message
  ...
  useEffect(() => {
    // --- FIX 2: Conditional call using the ref ---
  ...
  // --- Search Logic (rest of component functions remain the same) ---
  const fuseOptions = {
  ...
  const editForm = (form) => {
    const formData = {
      ...form,
      timming: JSON.stringify(form.timming || []),
