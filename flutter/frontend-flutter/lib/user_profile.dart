...
import 'profile.dart'; // Import ProfilePage for navigation
...
class _UserProfilePageState extends State<UserProfilePage> {
  ...
  Future<void> _fetchUserProfile() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(AppConfig.getUserProfile(userId)),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // FIX: Check for both 'phoneNumber' (standard) and 'phone' (your database field)
          ...
          if (userData['favorites'] is List) {
            final favoritesList = userData['favorites'] as List;
            
            if (favoritesList.isNotEmpty && favoritesList.first is String) {
                favoriteIds = List<String>.from(favoritesList.map((id) => id.toString()));
                _fetchFavoriteApplicants(); 
            } else if (favoritesList.isNotEmpty && favoritesList.first is Map) {
                favoriteApplicants = List<Map<String, dynamic>>.from(
                    favoritesList.map((favorite) => favorite as Map<String, dynamic>)
                );
          ...
          if (userData['timming'] is List) {
            timingPreferences = userData['timming'];
          } else if (userData['timing'] is List) {
            timingPreferences = userData['timing'];
          } else {
            timingPreferences = [];
          }
        });
  ...
  Future<void> _fetchFavoriteApplicants() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(AppConfig.getAllForms()),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> allApplicants = responseData['data'] ?? responseData;
        
        final List<Map<String, dynamic>> favorites = [];
        for (final applicant in allApplicants) {
          if (applicant is Map && favoriteIds.contains(applicant['_id'])) {
            favorites.add(Map<String, dynamic>.from(applicant));
          }
        }
  ...
  Future<void> _saveProfile() async {
    try {
      ...
      final response = await http.put(
        Uri.parse(AppConfig.updateUserProfile(userId!)),
        headers: {
          'Authorization': 'Bearer $token',
        ...
        body: jsonEncode({
          ...
          // CRITICAL: Save the phone number field used by the database
    ...
    try {
      ...
      if (isFavorite) {
        final response = await http.delete(
          Uri.parse(AppConfig.removeFromFavorites(userId!, applicantId)),
          headers: {
            'Authorization': 'Bearer $token',
          },
        );
        ...
        final response = await http.post(
          Uri.parse(AppConfig.addToFavorites(userId!)),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
          },
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              // FIX: Navigation to ProfilePage from Favorite list
              Navigator.push(
                context,
                MaterialPageRoute(
