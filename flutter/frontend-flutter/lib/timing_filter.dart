...
class _TimingFilterPageState extends State<TimingFilterPage> {
  ...
  // FIX: Ensured time slots use hyphens with spaces (e.g., "1 AM - 2 AM")
  // to match the format saved by the web form.
  final Map<String, List<String>> timingMap = {
    'Early Morning': [
      '12 AM - 1 AM', '1 AM - 2 AM', '2 AM - 3 AM', 
      '3 AM - 4 AM', '4 AM - 5 AM', '5 AM - 6 AM'
    ],
    'Morning': [
  ...
  Widget build(BuildContext context) {
    // --- Consistency Fix: Remove trailing space from the "Early Morning" key if it exists in the original Map
    // Although the map above is corrected, this prevents an issue if the map was copied incorrectly elsewhere.
    timingMap.remove('Early Morning ');
    timingMap['Early Morning'] = [
      '12 AM - 1 AM', '1 AM - 2 AM', '2 AM - 3 AM', 
      '3 AM - 4 AM', '4 AM - 5 AM', '5 AM - 6 AM'
    ];
              ...
              setState(() {
                if (selectedTimings.isEmpty) {
                  ...
                  for (var slots in timingMap.values) {
                    // Use a temporary Set to avoid adding duplicate slots in case of overlap logic changes
                    final allSlots = {...selectedTimings, ...slots}; 
                    selectedTimings = allSlots.toList();
                  }
                } else {
              ...
              onChanged: (bool? value) {
                setState(() {
                  if (value == true) {
                    ...
                    // Use Set conversion to prevent duplicates
                    final currentSelection = Set<String>.from(selectedTimings);
                    for (String slot in timeSlots) {
                      currentSelection.add(slot);
                    }
                    selectedTimings = currentSelection.toList();
                  } else {
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () {
                  // Ensure selectedTimings are applied (passed back to HomePage)
